...
const ProductCard: React.FC<ProductProps> = ({ title, img, price, oldPrice, stock, description, cuotas1, cuotas2 }) => {
  ...
  const whatRef: RefObject<HTMLDialogElement> = createRef();

  function openContactDialog() {
    document.body.style.overflow = "";
    whatRef.current?.showModal?.();
  }

  function closeContactDialog() {
    document.body.style.overflow = "";
    whatRef?.current?.close();
  }
  ...
  return (
    <div className={styles.card}>
      ...
      <ContactDialog onClose={closeContactDialog} whatsAppMessage={whatsAppMessage} dialogRef={whatRef} />
    </div>
  );
};
...
<Link href={`https://api.whatsapp.com/send?phone=+5491158072839&text=${whatsAppMessage}`} className={styles.whatsappLink}>
  <p className=" text-[#439c4c] text-sm">Lucas</p>
...
<Link href={`https://api.whatsapp.com/send?phone=+5491127087406&text=${whatsAppMessage}`} className={styles.whatsappLink}>
  <p className=" text-[#439c4c] text-sm">Marcos</p>
